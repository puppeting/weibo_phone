...
public class TsDialog extends Dialog {
    ...
    public TsDialog(Context context, int  threm ){
        super(context,threm);
    ...
    protected TsDialog(Context context, boolean cancelable, OnCancelListener cancelListener) {
    ...
    private void initView() {
        View view = LayoutInflater.from(context).inflate(R.layout.service_pop_layout, null, false);
        setContentView(view);
...
//        llTongYi.setText("去阅读");
        findViewById(R.id.tv_ok).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                OpenDialog mdialog = new OpenDialog(context, R.style.MyDialogStyle);
                mdialog.show();
                dismiss();
            }
        });
